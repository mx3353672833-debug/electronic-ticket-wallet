...
export async function createAccounts({dataDir,config,sendMail,now=()=>Date.now()}) {
  if(config.registration!=='invite') throw new Error('Only invitation-based registration is supported')
  ...
  const ownerOnly=user=>{if(user?.role!=='owner')throw failure(403,'仅站长可以管理邀请')}
  const invitationSummary=i=>({id:i.id,createdAt:i.createdAt,expiresAt:i.expiresAt,status:i.usedAt?'used':i.revokedAt?'revoked':i.expiresAt<=now()?'expired':'active'})
  const listInvitations=user=>{ownerOnly(user);return (store.read().invitations||[]).map(invitationSummary).reverse()}
  const createInvitation=async user=>{
    ownerOnly(user)
    const token=crypto.randomBytes(32).toString('base64url')
    return store.update(next=>{
      next.invitations??=[]
      if(next.invitations.filter(i=>!i.usedAt && !i.revokedAt && i.expiresAt>now()).length>=100 || next.invitations.length>=2000)throw failure(429,'邀请链接数量已达上限，请先撤销不需要的邀请')
      const invite={id:crypto.randomUUID(),hash:inviteHash(token),createdAt:now(),expiresAt:now()+7*86400000}
      next.invitations.push(invite)
      return {...invitationSummary(invite),token}
    })
  }
  const revokeInvitation=async(user,id)=>{
    ownerOnly(user)
    return store.update(next=>{const invite=(next.invitations||[]).find(i=>i.id===id);if(!invite)throw failure(404,'邀请不存在');if(!invite.usedAt)invite.revokedAt=now();return invitationSummary(invite)})
  }
  ...
  const sendCode=async({email:input,purpose,inviteCode},ip)=>{
    ...
    if(purpose==='register') requireInvite(store.read(),inviteCode)
    ...
    const reservation=await store.update(next=>{
      const invitation=purpose==='register'?requireInvite(next,inviteCode):null
      ...
      next.codes[key]={hash:codeHash(email,purpose,code),expires:time+600000,attempts:0,sent:false,...(invitation?{inviteHash:invitation.hash}:{})}
  ...
  const finishCode=async({email:input,code,password,inviteCode},purpose,ip)=>{
    ...
    const result=await store.update(async next=>{
      // Validate and consume in the same transaction as the user and OTP. Two emails
      // racing on a single link cannot both register, even across a service restart.
      const invitation=purpose==='register'?requireInvite(next,inviteCode):null
      ...
      if(invitation && record.inviteHash!==invitation.hash)return {error:'验证码与邀请链接不匹配，请重新获取验证码'}
      ...
      if(invitation){invitation.usedAt=time;invitation.usedBy=user.id}
  ...
  return {sendCode,register:(input,ip)=>finishCode(input,'register',ip),reset:(input,ip)=>finishCode(input,'reset',ip),login,getUser,publicUser,inviteValid,listInvitations,createInvitation,revokeInvitation}
}
