...
export default function App() {
  ...
  return <div className={`app-shell${trayCollapsed ? ' is-tray-collapsed' : ''}`} data-testid="ticket-wallet-home">
    <main className="map-stage"><Suspense fallback={<div className="app-loading">正在加载地图…</div>}><TicketMap fitRequest={fitRequest} /></Suspense></main>
    ...
    <button className="map-fit" onClick={() => setFitRequest(n=>n+1)} aria-label="查看所有线路" title="查看所有线路"><Icon name="globe" /></button>
    <footer className="collection-tray">
      ...
      <div className="collection-status"><span>{scanned} 张已扫描 · {REMOTE ? '私密云端票夹' : '仅保存在本机浏览器'}</span><span>蓝色：实际铁路路网推算，非历史车次确认轨迹</span></div>
      </div></div>
    </footer>
