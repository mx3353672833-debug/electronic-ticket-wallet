...
export default function App() {
  ...
  const routed = hits.filter(t => t.track || t.railRoute).length
  ...
  return <div className={`app-shell${trayCollapsed ? ' is-tray-collapsed' : ''}`} data-testid="ticket-wallet-home">
    ...
    <header className="app-header">
      ...
      <nav className="header-tools" aria-label="票夹工具"><ReleaseNotes />{REMOTE && <AccountTools />}<button className="toolbar-button" aria-label="扫描与识别票面" onClick={() => setProcessing(true)} disabled={!tickets.length}><Icon name="image" size={18} /><span>{REMOTE ? '整理工具' : scanned ? '重新整理' : '整理票面'}</span></button><button className="add-button" aria-label="上传票据照片" onClick={() => setUploadOpen(true)}><Icon name="plus" size={22} /><span>添加</span></button></nav>
    </header>
    ...
    <footer className="collection-tray">
      <div className="tray-toolbar"><YearTimeline /><button className="random-button" disabled={!hits.length} onClick={() => randomTicket()} aria-label="随机翻一张票" title="随机翻一张票"><Icon name="shuffle" size={17} /></button><span className="tray-count">{searchQuery.trim() ? hits.length + ' 张匹配' : yearRange ? hits.length + ' 张票 · ' + routed + ' 张有线路' : routed + ' 张已关联线路'}</span><button type="button" className="tray-toggle" aria-label={trayCollapsed ? '展开票据栏' : '收起票据栏'} aria-expanded={!trayCollapsed} aria-controls="ticket-tray-content" onClick={() => { const next = !trayCollapsed; setTrayCollapsed(next); saveTrayCollapsed(next) }}><Icon name={trayCollapsed ? 'chevronUp' : 'chevronDown'} size={17} /><span>{trayCollapsed ? '展开' : '收起'}</span></button></div>
      ...
      <div className="ticket-filmstrip" aria-label="所有票据">{hits.map(t=><button key={t.id} className="film-ticket" onPointerEnter={()=>warmTicketImage(t.processedImageUrl)} onFocus={()=>warmTicketImage(t.processedImageUrl)} onClick={e=>openTicket(t.id,e.currentTarget)} aria-label={formatRoute(t) + ' ' + (t.takenAt || t.sourceFile?.name || '日期待补充')}><img src={t.thumbnailUrl} alt="" loading="lazy" decoding="async"/><span>{t.takenAt || '日期待补充'}</span>{t.processing?.documentKind==='refund' && <small>退票凭证</small>}</button>)}{!hits.length && <p className="empty-collection">{tickets.length ? '没有匹配的票' : '添加票据照片，开始整理。'}</p>}</div>
      <div className="collection-status"><span>{REMOTE ? '私密云端票夹' : '仅保存在本机浏览器'}</span></div>
      </div></div>
    </footer>
