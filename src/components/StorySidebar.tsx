...
export function StorySidebar({ ticket, onDirty, onClose }: { ticket: Ticket; onDirty: (dirty: boolean) => void; onClose: () => void }) {
  ...
  return <aside className="story-sidebar" aria-label="旅程笔记"><div className="story-sidebar-header"><span>信息</span><button className="icon-button" aria-label="关闭旅程笔记" onClick={onClose}><Icon name="close" /></button></div><h2>旅程笔记</h2>
    ...
    <section className="track-section"><div className="track-section-title"><Icon name="map" /><h3>线路</h3></div>{ticket.track ? <><p className="track-info">{trackLength(ticket.track).toFixed(1)} km</p><p className="track-filename">{ticket.track.filename}</p></> : ticket.railRoute ? <><p>{ticket.carrierOrTrainNo} · {ticket.railRoute.distanceKm} km</p>{ticket.railRoute.timetable && <p className="route-stops">{ticket.railRoute.timetable.stops.join(' → ')}</p>}<a className="source-link" href={ticket.railRoute.sourceUrl} target="_blank" rel="noreferrer">{ticket.railRoute.attribution}</a>{ticket.railRoute.timetable && <p><a className="source-link" href="https://railgo.dev/" target="_blank" rel="noreferrer">经停数据 · RailGo</a></p>}</> : <p>{ticket.processing?.documentKind==='refund' ? '退票凭证' : '暂无线路，可更新车次线路或导入轨迹。'}</p>}
    {canUpdateRoute(ticket) && <button className="text-button" disabled={routing || saving || editing || importing} onClick={async()=>{
    ...
    <input className="sr-only" ref={fileInput} type="file" accept=".gpx,.geojson,.json" aria-label="选择轨迹文件" onChange={e => void importTrack(e.target.files?.[0])} /><button className="track-import" disabled={importing || saving || editing || routing} onClick={() => fileInput.current?.click()}><Icon name="upload" size={15} />{importing ? '正在读取轨迹…' : ticket.track ? '替换轨迹文件' : '导入 GPX / GeoJSON'}</button><small>{REMOTE ? '轨迹保存在私密服务器。' : '轨迹文件仅保存在当前浏览器。'}</small></section>
