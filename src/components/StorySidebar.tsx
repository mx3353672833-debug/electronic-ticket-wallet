...
export function StorySidebar({ ticket, onDirty, onClose }: { ticket: Ticket; onDirty: (dirty: boolean) => void; onClose: () => void }) {
  ...
  return <aside className="story-sidebar" aria-label="旅程笔记"><div className="story-sidebar-header"><span>信息</span><button className="icon-button" aria-label="关闭旅程笔记" onClick={onClose}><Icon name="close" /></button></div><h2>旅程笔记</h2>
    ...
    <section className="track-section"><div className="track-section-title"><Icon name="map" /><h3>线路</h3></div>{ticket.track ? <><p className="track-info">已导入轨迹 · {trackLength(ticket.track).toFixed(1)} km</p><p className="track-filename">{ticket.track.filename}</p></> : ticket.railRoute ? <><p>{ticket.railRoute.distanceKm} km · {ticket.railRoute.segments.reduce((n,s)=>n+s.length,0).toLocaleString()} 个实际路网点</p><p>{ticket.railRoute.lineNames.join(' / ') || 'OSM 铁路轨道'}</p><p className="route-caution">仅按起终站和普速／动车类别推算，未查询此车次及乘车日期。同站不同车次可能走不同线路，本线不能当作这趟车的真实经由。路网快照：{ticket.railRoute.datasetDate}。</p><a className="source-link" href={ticket.railRoute.sourceUrl} target="_blank" rel="noreferrer">{ticket.railRoute.attribution}</a></> : <p>{ticket.processing?.documentKind==='refund' ? '退票费凭证不自动生成行程。' : '还没有可确认的线路，不绘制起终点直线。可以导入实际行程轨迹。'}</p>}<input className="sr-only" ref={fileInput} type="file" accept=".gpx,.geojson,.json" aria-label="选择轨迹文件" onChange={e => void importTrack(e.target.files?.[0])} /><button className="track-import" disabled={importing || saving || editing} onClick={() => fileInput.current?.click()}><Icon name="upload" size={15} />{importing ? '正在读取轨迹…' : ticket.track ? '替换轨迹文件' : '导入 GPX / GeoJSON'}</button><small>{REMOTE ? '轨迹保存在私密服务器。' : '轨迹文件仅保存在当前浏览器。'}</small></section>
