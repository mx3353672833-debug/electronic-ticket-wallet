...
export function SearchBar() {
  ...
  return <div className="search-bar" onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setOpen(false) }}>
    <div className="search-input-wrap"><Icon name="search" /><input ref={input} type="search" name="ticket-search" autoComplete="off" aria-label="搜索票据" placeholder="搜索地点、日期、车次或故事" value={searchQuery} onFocus={() => setOpen(true)} onChange={e => { setSearchQuery(e.target.value); setActive(0); setOpen(true) }} onKeyDown={e => {
      ...
      if (e.key === 'Enter' && shown[active]) { openTicket(shown[active].id); setOpen(false) }
    ...
    {open && searchQuery.trim() && <div className="search-results" aria-label="搜索结果"><div className="search-result-label">{results.length ? `找到 ${results.length} 张票` : '没有匹配的票'}</div>{shown.map((ticket, i) => <button key={ticket.id} className={`search-result-item ${i === active ? 'active' : ''}`} onPointerDown={e => e.preventDefault()} onClick={() => { openTicket(ticket.id); setOpen(false) }}><span>{formatRoute(ticket)}</span><small>{ticket.takenAt || '日期待补'} · {ticket.carrierOrTrainNo || '旅程'}</small><Icon name="arrow" size={16} /></button>)}</div>}
  </div>
}
