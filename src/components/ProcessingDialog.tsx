...
export function ProcessingDialog({onClose}:{onClose:()=>void}) {
  ...
  return <Modal className="upload-modal batch-modal" label="扫描与识别" onClose={close}>
    ...
    <div className="batch-body"><p className="processing-intro">自动裁边、透视校正，识别日期、站点和车次。原始照片和已有故事不会被替换。</p><p className="form-note">{REMOTE ? '使用私密服务器的 OpenCV + Tesseract。已扫描的票只补齐统一外观，不重复识别；已整理好的票保持不变。' : '使用这台 Mac 的本地扫描服务，'}不调用生成式 AI。只提示缺少的信息，不需要逐张确认。</p>
