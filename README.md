...
- 本机 IndexedDB 收藏；或独立部署带登录保护的单用户服务器版。
...
见 [自托管说明](docs/self-hosting.md)。服务器版使用 Node.js + OpenCV + Tesseract，网页、照片和 API 均要求登录。默认部署路径为 `/tickets/`，必须通过 HTTPS 反向代理使用。
...
票据通常包含姓名、证件信息和出行记录。请勿在公开 Issue、截图、示例或仓库中上传未脱敏内容。本项目不是多租户服务，也未经过独立安全审计。
