...
for (const name of ['production.mjs','accounts.mjs','auth-page.mjs','auth.js','auth.css','collection.mjs','private-store.mjs','mail.mjs','feedback.mjs','train-routes.mjs','appearance.mjs','display-images.mjs']) {
  await fs.copyFile('server/'+name,'.local-data/deploy/server/'+name)
}
for (const name of ['scan-ticket-linux.py','init-wallet.mjs','upgrade-accounts.mjs','rail-itinerary.py','rail_graph.py','ticket-appearance.py','display-image.py','warm-display-images.mjs']) {
